    ...
    if (e.target.classList.contains('comments-btn')) {
      ...
      try {
        ...
        comments.forEach(comment => {
          ...
          commentEl.innerHTML = `
            <img src="${comment.author_avatar || '/img/svg/user.svg'}" alt="${comment.author_name}" class="author-avatar">
...
async function initProfile() {
  ...
  const profileId = urlParams.get('id') || (currentUser ? currentUser.id : null);
  ...
  try {
    ...
    document.getElementById('profile-avatar').src = profile.avatar_url || '/img/svg/user.svg';
    ...
    document.getElementById('edit-name-btn').style.display = isOwnProfile ? 'inline-block' : 'none';
    document.getElementById('edit-description-btn').style.display = isOwnProfile ? 'inline-block' : 'none';
    document.getElementById('change-avatar-btn').style.display = isOwnProfile ? 'inline-block' : 'none';
    
    // Загрузка постов
    const posts = await fetchData(`/api/user/${profileId}/posts`);
    renderPosts(posts, document.getElementById('posts-container'));
    
    // Инициализация редактора (для своего профиля)
    if (isOwnProfile) {
      initProfileEditor();
      initPostCreator();
    }
...
function createPostElement(post) {
  ...
  postEl.innerHTML = `
    <header class="card-header">
      <img class="author-avatar" src="${post.author_avatar || '/img/svg/user.svg'}" alt="${post.author_name}">
    ...
    <footer class="card-footer">
      <button class="like-btn ${post.liked ? 'liked' : ''}" aria-label="Лайк">
        <img src="/img/svg/${post.liked ? 'like-filled' : 'like'}.svg" width="24" height="24" alt="">
        <span class="like-count">${post.likes_count}</span>
      </button>
      <button class="comments-btn" aria-label="Комментарии">Комментарии ${post.comments_count}</button>
...
function initSearch() {
  ...
  let currentType = window.location.pathname.includes('users') ? 'users' : 'posts';
  ...
  function renderResults(results, type) {
    ...
    if (type === 'users') {
      results.forEach(user => {
        ...
        userEl.innerHTML = `
          <img src="${user.avatar_url || '/img/svg/user.svg'}" alt="${user.name}" class="author-avatar">
          <a href="/profile.html?id=${user.id}" class="user-name">${user.name}</a>
        `;
      ...
      results.forEach(post => {
        ...
        postEl.innerHTML = `
          <div class="post-author">
            <img src="${post.author_avatar || '/img/svg/user.svg'}" alt="${post.author_name}" class="author-avatar">
            <span class="author-name">${post.author_name}</span>
          </div>
          <p>${post.content}</p>
          <a href="/index.html#post-${post.id}" class="post-link">Перейти к посту</a>
        `;
